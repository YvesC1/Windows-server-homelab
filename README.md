...
3. Click **Upload** and upload both the Windows Server 2022 and Windows 11 ISOs.
...
5. **Disks tab:** Storage: `local-lvm` · Disk size: **50 GB** · Format: Raw disk image (or qcow2)
...
9. Review the **Confirm** summary and click **Finish**.
...
6. Select unallocated space (50 GB) → **Next**.
7. Upon reboot, set the built-in Administrator password: `Password1`.
8. Click **Finish** and log in via Console (Ctrl+Alt+Del button at top-left of noVNC).
...
6. Accept the EULA → **Custom: Install Windows only (advanced)** → select the 64 GB partition → **Next**.
7. Complete the Out-of-Box Experience (OOBE):
